...
def genSequenceLearning(groupname = 'Seq',
    ...
    indInput = np.asarray([])
    gInputGroup = SpikeGeneratorGroup(numNeuronsPerGroup, indices=indInput, times=tsInput)
    # CoS Input #TODO: CoS as NeuronGroup 
    tsCoS  = np.asarray([]) * ms
    indCoS = np.asarray([])
    gCoSGroup = SpikeGeneratorGroup(numNeuronsPerGroup, indices=indCoS, times=tsCoS)
    # reset group #TODO: Reset as NeuronGroup 
    tsReset  = np.asarray([]) * ms
    indReset = np.asarray([])
    gResetGroup = SpikeGeneratorGroup(numNeuronsPerGroup, indices=indReset, times=tsReset)
    ...
    # Neurongoups
    gOrdGroups = NeuronGroup(nOrdNeurons,  name='g' + groupname +'Ord', **neuronEqsDict_Ord)
    gMemGroups = NeuronGroup(nMemNeurons,  name='g' + groupname +'Mem', **neuronEqsDict_Mem)
    ...
    # Monitors
    if monitor:
        spikemonOrd = SpikeMonitor(gOrdGroups)
        spikemonMem = SpikeMonitor(gMemGroups)
        spikemonCoS = SpikeMonitor(gCoSGroup)
        spikemonInp = SpikeMonitor(gInputGroup)
        spikemonReset = SpikeMonitor(gResetGroup)
    else:
        spikemonOrd = False
        spikemonMem = False
        spikemonCoS = False
    ...
    return ((gOrdGroups,gMemGroups,gInputGroup,gCoSGroup,gResetGroup,synInpOrd1e,synOrdMem1e,synMemOrd1e,synOrdOrd1e,synMemMem1e,synOrdOrd1i,synMemOrd1i,
            synCoSOrd1i,synResetOrd1i,synResetMem1i,spikemonOrd,spikemonMem,spikemonCoS,spikemonInp,spikemonReset))
