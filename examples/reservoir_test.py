...
nc17 ={}
loadmat('config2.mat',nc17) # config2.mat generated by IZFORCECLASSIFICATION_randcontrol.m
...
adjecency_mtr[:,:,1] = 0 #nc17['OMEGA'].T
...
for cinj in gtestR.Monitors['statemonR'].Iconst:
    ...
    plt.plot(gtestR.Monitors['statemonR'].t/ms,nc17['current_I'],'r')
    plt.xlabel('Time (ms)')
...
plt.plot(gtestR.Monitors['statemonR'].t/ms, nc17['current_r']-0.001,'r')
plt.xlabel('Time (ms)')
plt.ylabel('Rate')
...
plt.plot(gtestR.Monitors['statemonR'].t/ms, nc17['current_v']-1,'r')
plt.xlabel('Time (ms)')
plt.ylabel('Rate')
...
error = np.array(error)
plt.plot(gtestR.Monitors['statemon_readout_rate'].t/ms, gtestR.Monitors['statemon_readout_rate'].rate.reshape((25000,)))
plt.plot(np.arange(0,nc17['T'],nc17['dt']), nc17['zx'],'r')
plt.xlabel('Time (ms)')
plt.ylabel('Error')
