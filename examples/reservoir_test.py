...
# nc17_ml ={}
# loadmat('config10.mat',nc17_ml) # config2.mat generated by IZFORCECLASSIFICATION_randcontrol.m

# # nc17_ml['N'] = nc17_ml['N'][0][0]
# # nc17_ml['E'] = nc17_ml['E'].reshape((nc17_ml['N']))
# # nc17_ml['zx'] = nc17_ml['zx'].T
# # nc17_ml['BPhi'] = nc17_ml['BPhi'].reshape((nc17_ml['N']))
# # nc17_ml['v_init'] = nc17_ml['v_init'].reshape((nc17_ml['N'])).T
# # nc17_ml['dt'] = nc17_ml['dt'][0][0]
# # nc17_ml['zx'] = nc17_ml['zx'].T[0]
# # nc17_ml['T'] = 300
# # for pk in nc17.keys():
# #     if pk in nc17_ml.keys() and 'current' not in pk and 'record' not in pk:
# #         print (pk,)
# #         if isinstance(nc17[pk],np.ndarray):
# #             if len(nc17_ml[pk].shape) > 1:
# #                 if (nc17[pk]- nc17_ml[pk][0:nc17[pk].shape[0],:]).any():
# #                     print(nc17[pk]- nc17_ml[pk][0:nc17[pk].shape[0],:])
# #             else:
# #                 if (nc17[pk]- nc17_ml[pk][0:nc17[pk].shape[0]]).any():
# #                     print (nc17[pk]- nc17_ml[pk][0:nc17[pk].shape[0]])
# #         else:
# #             if (nc17[pk]- nc17_ml[pk]):
# #                 print (nc17[pk]- nc17_ml[pk])
# #         # print (pk, nc17[pk], nc17_ml[pk])
# #     else:
# #         print ('%s not in matlab'%pk)

# nc17 = nc17_ml
# nc17['N'] = nc17['N'][0][0]
# nc17['E'] = nc17['E'].reshape((nc17['N']))
# nc17['zx'] = nc17['zx'].T
# nc17['BPhi'] = nc17['BPhi'].reshape((nc17['N']))
# nc17['BPhi'] = np.ones((nc17['N'],1)).reshape((nc17['N'])) * 0.000001
# nc17['v_init'] = nc17['v_init'].reshape((nc17['N'])).T
# nc17['dt'] = nc17['dt'][0][0]
# nc17['zx'] = nc17['zx'].T[0]
# nc17['T'] = 300
...
gtestR.Groups['gRGroup'].Vm = nc17['v_init'].T * mV
...
def FORCE(t):
    if t > (nc17['imin']-1)*nc17['dt']*ms and t < nc17['icrit']*nc17['dt']*ms:
        ...
        # cd = Pinv*r
        cd = np.dot(nc17['Pinv'], firing_rate).reshape((nc17['N'],1))
        cxd.append(cd.T.tolist()[0])
        # print("UPDATE!!",t.variable.get_value_with_unit(),t/nc17['dt']/ nc17['step']/ms, t/nc17['dt']/ms)
        # ipdb.set_trace()
        if log: print('cd',cd)
        #    BPhi = BPhi - (cd*err')
        ...
        # print(t.variable.get_value_with_unit(),t/nc17['dt']/ nc17['step']/ms, t/nc17['dt']/ms)
        firing_rate = np.array(gtestR.Groups['synOutR1e'].r)
        read_out = np.array(gtestR.Groups['gROutGroup'].rate)[0]
        z = read_out #np.dot(nc17['BPhi'].T,r)
        zx = nc17['zx'][round(t/nc17['dt']/ms)]
        err = z - zx # scalar error
        error.append([np.array(t.variable.get_value_with_unit())[0], err])
        cxd.append(np.zeros_like(nc17['BPhi']).tolist())
        BPhi.append(nc17['BPhi'].tolist())
        BPhi_updates.append(t/ms)

# BIAS, Input, and Feedback
# Input Ein*Xin
...
plt.plot(gtestR.Monitors['statemon_readout_rate'].t/ms, gtestR.Monitors['statemon_readout_rate'].rate.T-0.01)
plt.plot(nc17['dt'] * np.cumsum(np.ones_like(nc17['zx'])), nc17['zx']-0.01,'k')
...
plt.plot(nc17_data['record_update_times'] * nc17_data['dt'], nc17_data['record_BPhi'],'g')
# plt.plot(nc17_data['time'].T, nc17_data['zx'].T,'r')
plt.xlabel('Time (ms)')
plt.ylabel('Weights')
