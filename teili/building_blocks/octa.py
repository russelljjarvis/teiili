...
class Octa(BuildingBlock):
    ...
    def __init__(self, name='octa*',
        ...
        block_params = {}
        block_params.update(wta_params)
...
def gen_octa(groupname,
             neuron_eq_builder
             num_input_neurons,
             num_neurons, wta_params, octa_params, ,
             external_input=True, noise=True,
    ...
    replace_neurons(compressionWTA, 'spike_gen' ,num_input_neurons**2 ,\
                         equation_builder = neuron_eq_builder, refractory = wta_params['rp_exc'])
    compressionWTA._set_tags(tags.basic_wta_n_sg, compressionWTA._groups['spike_gen'])
    ...
    compressionWTA._set_tags(tags.basic_wta_s_inp_exc, compressionWTA._groups['s_inp_exc'])
    compressionWTA._groups['s_inp_exc'].weight =wta_params['we_inp_exc']
    compressionWTA._groups['s_inp_exc'].taupre = octa_params['tau_stdp']
    compressionWTA._groups['s_inp_exc'].taupost = octa_params['tau_stdp']
    ...
    compressionWTA._set_tags(tags.basic_wta_s_exc_exc, compressionWTA._groups['s_exc_exc'])
    compressionWTA._groups['s_exc_exc'].weight = wta_params['we_exc_exc']
    ...
    compressionWTA._set_tags(tags.basic_wta_s_inh_exc,
                             compressionWTA._groups['s_inh_exc'])

    compressionWTA._groups['s_inh_exc'].weight = wta_params['wi_inh_exc']
    compressionWTA._groups['s_inh_exc'].variance_th = np.random.uniform(low=octa_params['variance_th_c']-0.1,
                                                                 high=octa_params['variance_th_c']+0.1,
    ...
    replace_connection(predictionWTA, 'n_inh', predictionWTA, 'n_exc',\
                       's_inh_exc', equation_builder= DPIadp)
    predictionWTA._set_tags(tags.basic_wta_s_inh_exc, predictionWTA._groups['s_inh_exc'])
    predictionWTA._groups['s_inh_exc'].weight = wta_params['wi_inh_exc']
    predictionWTA._groups['s_inh_exc'].variance_th = np.random.uniform(low=octa_params['variance_th_p']-0.1,
                                                                 high=octa_params['variance_th_p']+0.1,
...
#    #Modify the input of the prediction WTA. Bypassing the spike_gen block
    ...
    predictionWTA._set_tags(tags.basic_wta_s_inp_exc, predictionWTA._groups['s_inp_exc'])
    ...
    predictionWTA._groups['s_inp_exc'].weight = wta_params['we_inp_exc']
    predictionWTA._groups['s_inp_exc'].taupre = octa_params['tau_stdp']
    predictionWTA._groups['s_inp_exc'].taupost = octa_params['tau_stdp'] 
    ...
    compressionWTA._set_tags(tags.basic_wta_s_exc_exc, predictionWTA._groups['s_exc_exc'])
    predictionWTA._groups['s_exc_exc'].weight = wta_params['we_exc_exc']
    ...
    compressionWTA._groups['s_inh_exc'].inh_learning_rate = octa_params['inh_learning_rate']
    predictionWTA._groups['s_inh_exc'].inh_learning_rate = octa_params['inh_learning_rate']
    ...
    add_weight_init(w_init_group,
                    dist_param=octa_params['dist_param_init'],
                    scale=octa_params['scale_init'],
                    distribution=octa_params['distribution'])
    ...
    add_decay_weight(weight_decay_group,
                     decay_strategy=octa_params['weight_decay'],
                     learning_rate=octa_params['learning_rate'])
    ...
    add_weight_re_init(weight_re_init_group,
                       re_init_threshold=octa_params['re_init_threshold'],
                       dist_param_re_init=octa_params['dist_param_re_init'], 
                       scale_re_init=octa_params['scale_re_init'],
                       distribution=octa_params['distribution'])
    ...
    # add_regulatization_weight(weight_regularization_group, buffer_size=octa_params['buffer_size'] )


    activity_proxy_group = [compressionWTA._groups['n_exc']] + \
                           [predictionWTA._groups['n_exc']]

    add_proxy_activity(activity_proxy_group,
                       buffer_size=octa_params['buffer_size_plast'],
                       decay=octa_params['decay'])
    ...
    if external_input is True:
        ...
        testbench_stim.rotating_bar(length=10, nrows=10, 
                                ...
                                noise_probability=0.2, repetitions=octa_params['revolutions'], debug=False)
    ...
    if noise:
        ...
        noise_syn_c_exc.weight = octa_params['noise_weight']
        ...
        noise_syn_p_exc.weight = octa_params['noise_weight']
    ...
    octa_params['duration'] = np.max(testbench_stim.times)
...
def replace_neurons(bb, population, num_inputs, equation_builder, refractory):
    ...
    equation rule
    '''
    bb._groups[population] = Neurons(num_inputs,
                                     equation_builder=equation_builder(num_inputs=4),
...
def error_prediction_connections(compressionWTA, predictionWTA,wta_params, octa_params):
    '''
    ...
    error_connection.weight = wta_params['we_inp_exc']
    error_connection.taupre = octa_params['tau_stdp']
    error_connection.taupost = octa_params['tau_stdp']
    ...
    # Set learning rate
    compressionWTA._groups['s_inp_exc'].dApre = octa_params['learning_rate']
    compressionWTA._groups['s_exc_exc'].dApre = octa_params['learning_rate']
    predictionWTA._groups['s_inp_exc'].dApre = octa_params['learning_rate']
    predictionWTA._groups['s_exc_exc'].dApre = octa_params['learning_rate']

    error_connection.dApre = octa_params['learning_rate']
    prediction_connection.dApre = octa_params['learning_rate']
...
'''
    self._set_tags(tags.basic_octa_error_connection, _groups['error_connection'])
    self._set_tags(tags.basic_octa_prediction_connection, _groups['prediction_connection'])
    self._set_tags(tags.basic_octa_inputSyn, _groups['inputSynapse'])
    self._set_tags(tags.basic_octa_inpGroup, _groups['inputGroup'])
    self._set_tags(tags.basic_octa_pred_noise_syn_exc, _groups['pred_noise_syn_exc'])
    self._set_tags(tags.basic_octa_comp_noise_syn_exc, _groups['comp_noise_syn_exc'])
    self._set_tags(tags.basic_octa_pred_noise_gen, _groups['pred_noise_gen'])
    self._set_tags(tags.basic_octa_comp_noise_gen, _groups['comp_noise_gen'])
...
if __name__ == '__main__':
    ...
    test_OCTA =  Octa(name='test_OCTA',
                      ...
                      octa_params = octa_params,
                      neuron_eq_builder=octa_neuron,
                      num_input_neurons= octa_params['num_input_neurons'],
                      num_neurons = octa_params['num_neurons'],
                      stacked_inp = True,
                      noise= True,
                      monitor=True,
                      debug=False)
