...
def weight_regularization(weights, source_N, target_N, mean_variance_time):
    """This function regularizes a given weight matrix using either the L1 or
    the L2 norm.

    To ensure stability during training we use the well-known L1 and L2 norms.
    Both norms act in parallel, however, only one is used at any given step.
    To decide if L1 or L2 should be used, we

    Since weights tend to form useful receptive fields, we would like to have
    additional regularizer operating on the synaptic weights on a shorter
    time-scale than weight re-initialization. To do so we introduce two regularization
    terms (i) L1 and (ii) L2.
    The L2 regularizer tries to push weights towards the mean of the weight distribution,
    whereas L1 will push the weights away from the mean towards the extreme values,
    e.g. 0 and 1.
    ...
    Note:
        ...
        Note, that L1 will always push the weight away from the average, whereas
        L2 tries to push it towards the average. These two effect can be described
        by facilitation vs. exploration of weights.
    ...
    Example:
       ...
       >>> # Now we can add the run_regularly function
       >>> syn_obj.namespace.update({'weight_regularization': weight_regularization})
       >>> syn_obj.run_regularly(''' w_plast = weight_regularization(w_plast, N_pre, N_post, mean_variance_time)''',
                                 dt=200*ms)

    Note:
        This run_regularly function depends on the mean_variance_time buffer which is
        calculated using the correlation_coefficient_tracking function (see below).

    Args:
        weights (np.ndarray. required): Flattened weight matrix.
        source_N (int, required): Number of units in the source population.
        target_N (int, required): Number of units in the target population.
        mean_variance_time (syn_obj.array): An array which holds the average
            variance over the N time steps of the.

    Returns:
        ndarray: Regularized flattened weight matrix.
    """
    data = np.reshape(weights, (source_N, target_N))
    bias = 0.001
    learning_rate_norm = 0.001
    if np.sum(mean_variance_time == 0) == 0:
        avg_slope = np.sum(np.diff(np.log(mean_variance_time))) / (mean_variance_time.size - 1)
        if avg_slope < 0.01:
            avg = np.true_divide(data.sum(0), (data != 0).sum(0))
            normalization_factor = data / avg  # L1 push everyting towards 0 and 1
            # print('L1: {} - {}'.format(np.min(normalization_factor), np.max(normalization_factor)))
        else:
            data += bias
            avg = np.mean(data, axis=0)
            normalization_factor = avg / data  # L2 pull everythin towards the mean
            # print('L2: {} - {}'.format(np.min(normalization_factor), np.max(normalization_factor)))
        update_factor = np.abs(1 - normalization_factor)
        weight_update = ((1 + (update_factor * learning_rate_norm)) * (normalization_factor >= 1)
                         ) + ((1 - (update_factor * learning_rate_norm)) * (normalization_factor < 1))
        # print('Weight update: {} - {}'.format(np.min(weight_update), np.max(weight_update)))
        data *= weight_update
