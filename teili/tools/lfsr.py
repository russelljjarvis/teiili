...
def create_lfsr(neuron_groups, synapse_groups, time_step):
    ...
    for g in groups:
        if isinstance(g, Neurons):
            lfsr_lengths.extend(list(g.rand_num_bits))
        elif isinstance(g, Connections):
            lfsr_lengths.extend(list(g.rand_num_bits_syn))
            if hasattr(g, 'decay_probability_Apre'):
                lfsr_lengths.extend(list(g.rand_num_bits_Apre))
                lfsr_lengths.extend(list(g.rand_num_bits_Apost))
            if hasattr(g, 'counter_Apre'):
                lfsr_lengths.extend(list(g.rand_num_bits_condApre1))
                lfsr_lengths.extend(list(g.rand_num_bits_condApre2))
                lfsr_lengths.extend(list(g.rand_num_bits_condApost1))
                lfsr_lengths.extend(list(g.rand_num_bits_condApost2))
    rand_num_bits = np.unique(lfsr_lengths)
    lfsr_max = [int(2**n - 1) for n in rand_num_bits]
    ...
    for i, n_bits in enumerate(rand_num_bits):
    ...
    for g in (groups):
        if isinstance(g, Neurons):
            g.lfsr_max_value, g.lfsr_init, g.seed, prev_index = (
                    get_parameters(len(g.rand_num_bits), lfsr, prev_index)
            )
            ...
            g.lfsr_max_value_syn, g.lfsr_init_syn, g.seed_syn, prev_index = (
                    get_parameters(len(g.rand_num_bits_syn), lfsr, prev_index)
            )
            ...
            if hasattr(g, 'decay_probability_Apre'):
                g.lfsr_max_value_Apre, g.lfsr_init_Apre, g.seed_Apre, prev_index = (
                        get_parameters(len(g.rand_num_bits_Apre), lfsr, prev_index)
                )
                ...
                g.lfsr_max_value_Apost, g.lfsr_init_Apost, g.seed_Apost, prev_index = (
                        get_parameters(len(g.rand_num_bits_Apost), lfsr, prev_index)
                )
            ...
            if hasattr(g, 'counter_Apre'):
                g.lfsr_max_value_condApre1, g.lfsr_init_condApre1, g.seed_condApre1, prev_index = (
                        get_parameters(len(g.rand_num_bits_condApre1), lfsr, prev_index)
                )
                g.lfsr_max_value_condApre2, g.lfsr_init_condApre2, g.seed_condApre2, prev_index = (
                        get_parameters(len(g.rand_num_bits_condApre2), lfsr, prev_index)
                )
                g.lfsr_max_value_condApost1, g.lfsr_init_condApost1, g.seed_condApost1, prev_index = (
                        get_parameters(len(g.rand_num_bits_condApost1), lfsr, prev_index)
                )
                g.lfsr_max_value_condApost2, g.lfsr_init_condApost2, g.seed_condApost2, prev_index = (
                        get_parameters(len(g.rand_num_bits_condApost2), lfsr, prev_index)
                )
