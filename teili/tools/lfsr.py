"""This file contains a colletion of functions that are used when
a LFSR is used as random number generator. This is particularly important
in stochastic models such as the one described by Wang, Thakur and van Schaik
(2018).
"""
...
def create_lfsr(neuron_groups, synapse_groups, time_step):
    ...
    for g in groups:
        if isinstance(g, Neurons):
            ...
            # Following condition is true only when there is STDP
    ...
    for g in (groups):
        if isinstance(g, Neurons):
            g.lfsr_max_value, g.lfsr_init, g.seed, prev_index = (
                    get_parameters(g.N, lfsr, prev_index)
            )
