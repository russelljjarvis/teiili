# -*- coding: utf-8 -*-
""" A collection of run_regular functions including weight_initilization,
...
def re_init_weights(weights,
    ...
    Args:
        ...
        re_init_index (array, bool, optional): Boolean index array indicating
            which weights need to be re-initilized. If None weights are updated
            based on average post-synaptic weight. re_init_index can be optained
            using get_re_init_index run_regularly.
...
def re_init_taus(taus,
    ...
    Args:
        taus (np.ndarray. required): Flattened synaptic time constant matrix.
        re_init_index (np.ndarray, required): Boolean array of rows to be updated in matrix
        source_N (int, required): Number of units in the source population.
        target_N (int, required): Number of units in the target population.
        dist_param (float, optional): Shape factor of gamma, or mean of
            normal distribution from which weights are sampled.
    ...
    # Re-initializing weights with normal distribution
    if dist == 1:
        data[:, re_init_index] = np.reshape(np.random.gamma(shape=dist_param, scale=scale,
                                                            size=source_N * np.sum(re_init_index)),
                                            (source_N, np.sum(re_init_index)))
    if dist == 0:
        data[:, re_init_index] = np.reshape(np.random.normal(loc=dist_param, scale=scale,
                                                             size=source_N * np.sum(re_init_index)),
                                            (source_N, np.sum(re_init_index)))
...
@implementation('numpy', discard_units=True)
@check_units(Imem=volt, buffer_pointer=1, membrane_buffer=1, kernel=1, result=volt)
def get_activity_proxy_vm(Vm, buffer_pointer, membrane_buffer, kernel):
    ...
    Example:
        ...
        >>> LIFvar = NeuronEquationBuilder.import_eq('octa/models/equations/LIFvar',
                                                         num_inputs=2)
        >>> Alphavdp = SynapseEquationBuilder.import_eq('octa/models/equations/Alphavdp')

        >>> neuron_obj = Neurons(2, equation_builder=LIFvar,
                                 name="neuron_obj")
        >>> syn_obj = Connections(neuron_obj,
                                  neuron_obj,
                                  equation_builder=Alphavdp,
                                  method='euler',
                                  name='syn_obj')
        ...
        >>> neuron_obj.run_regularly('''buffer_pointer = (buffer_pointer + 1) % buffer_size
            activity_proxy = get_activity_proxy(Vm, buffer_pointer, membrane_buffer, decay)''', dt=1*ms)
...
def get_re_init_index(weights, source_N, target_N, re_init_threshold, lastspike, t):
    ...
    resampled accordingly a boolen vector is required to indictate the update,
...
@implementation('numpy', discard_units=True)
@check_units(Vthr=volt, Vm=volt, EL=volt, VT=volt, sigma_membrane=volt, not_refractory=1, result=volt)
