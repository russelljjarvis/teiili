...
test_matrix = np.clip(test_matrix, 0, 0.75)
shuffled_matrix = np.zeros((4,4)) * np.nan
...
s = SortMatrix(nrows=4, ncols=4, axis=1, matrix=copy.deepcopy(shuffled_matrix))

plt.figure()
plt.title('Constructed matrix (by hand)')
plt.imshow(test_matrix)
plt.colorbar()

plt.figure()
plt.title('Unsorted matrix')
...
plt.imshow(shuffled_matrix[:, s.permutation], vmin=0, vmax=1)
plt.colorbar()
plt.show()
