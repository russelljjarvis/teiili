...
class orcaColumn:
    ...
    def create_layers(self, pop_modifier={}, conn_modifier={}):
        ...
        for layer in self.layers:
            conn_desc = ConnectionDescriptor(layer, self.model_path, 'intra')
            if conn_modifier[layer]:
                conn_desc.change_parameters(conn_modifier[layer])
            else:
                conn_desc.filter_params()

            pop_desc = PopulationDescriptor(layer, self.model_path)
    ...
    def connect_layers(self, conn_modifier={}):
        ...
        _ = [conn_modifier.setdefault(x) for x in pairs]

        for pair in pairs:
            laminar_source, laminar_target = pair[0], pair[1]
            conn_name = laminar_source+'_'+laminar_target
            conn_desc = ConnectionDescriptor(conn_name, self.model_path, 'inter')
            if conn_modifier[pair]:
                conn_desc.change_parameters(conn_modifier[pair])
            else:
                conn_desc.filter_params()
    ...
    def connect_inputs(self, input_group, input_name, conn_modifier={}):
        ...
        for layer in self.layers:
            conn_desc = ConnectionDescriptor(layer, self.model_path, 'input')
            if conn_modifier[layer]:
                conn_desc.change_parameters(conn_modifier[layer])
            else:
                conn_desc.filter_params()
